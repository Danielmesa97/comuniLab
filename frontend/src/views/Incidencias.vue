...
export default {
  ...
  const res = await fetch('http://127.0.0.1:8000/api/incidencias', {
    headers: {
  ...
  this.incidencias = data
    ...
    puedeEditar(i) {
      return (
        ...
        ['admin','presidente','superadmin'].includes(this.user.role)
      )
    },
    ...
    async cambiarEstado(i) {
      ...
      const res = await fetch(`http://127.0.0.1:8000/api/incidencias/${i.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
